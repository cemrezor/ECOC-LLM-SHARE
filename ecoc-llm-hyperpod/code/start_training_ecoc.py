...
from ecoc_model import EcocGPT2
import math
import numpy as np
...
def main():
    ...
    model = EcocGPT2(model_config, device=device)
    model = model.to(device)
    optim = torch.optim.Adam(model.parameters(), lr=model_config["lr"])

    run_name = f"{config['wandb']['prefix']}-ecoc-training-{args.model_config}-vocab-{model_config['vocab_size']}-epochs-{model_config['epochs']}"
